...
import { bindingsOf, commands, layout, LINKS_DOC_ID, split, tile } from "@hyperslop-systems/workbench-core";
...
const ordersApp = defineWorkbenchApp({
  manifest: { id: "orders", ports: [{ name: "order", direction: "out", contract: { valueType: "order", semanticRole: "order.current" }, doc: "the clicked order", drivesContext: "workspace.order" }] },
  presentation: { title: "orders", tone: "var(--pbui-cat-1)", Component: () => null },
});

const detailApp = defineWorkbenchApp({
  manifest: { id: "detail", ports: [{ name: "order", direction: "in", contract: { valueType: "order", semanticRole: "order.detail" }, doc: "the order shown", fallbackContext: "workspace.order" }] },
  presentation: { title: "detail", tone: "var(--pbui-cat-2)", Component: () => null },
});
...
function twoTiles() {
  ...
  const tree = () => workspaceTree(wb.core.getState().document, wb.core.getState().session.workspaceId);
...
describe("follow, pin, resume through the workbench", () => {
  it("port.follow writes the pbui.links payload and the badge reads → Orders East", () => {
    ...
    const payload = wb.core.getState().document.documents[LINKS_DOC_ID];
  ...
  it("an unlinked detail reads the context the table drives (scene 1, ambient)", () => {
    ...
    expect(wb.core.getState().document.documents[LINKS_DOC_ID]).toBeUndefined();
  });
  ...
  it("the verbs are workbench commands: validated, described, planned atomically with layout changes", () => {
    const { wb, orders, detail } = twoTiles();
    const verb = linkVerbs.follow(`${orders}/order`, `${detail}/order`) as Extract<ReturnType<typeof linkVerbs.follow>, { kind: "port.follow" }>;
    const bad = wb.execute([verb, commands.close("missing")]);
    expect(bad.ok).toBe(false);
    expect(wb.core.getState().document.documents[LINKS_DOC_ID]).toBeUndefined();
    const good = wb.execute([commands.duplicate(leaves(workspaceTree(wb.core.getState().document, wb.core.getState().session.workspaceId))[0]!.id, "row"), verb]);
    expect(good.ok).toBe(true);
    expect(bindingsOf(wb.core.getState().document).size).toBe(1);
    expect(viewsOfApp(wb.core.getState().document, "orders")).toHaveLength(2);
  });
});
...
describe("lifecycle in the same batch", () => {
  it("closing the source freezes the follower on its last value, with a resume that explains itself", () => {
    ...
    act(() => void wb.perform(linkVerbs.follow(`${orders}/order`, `${detail}/order`)));
    act(() => wb.links.runtime.emit(`${orders}/order`, ORDER));
    const placement = leaves(workspaceTree(wb.core.getState().document, wb.core.getState().session.workspaceId))[0]!.id;
    expect(performed(() => wb.execute(commands.close(placement)).ok)).toBe(true);
    expect(badgeTexts()).toEqual(["held:⏸#1042"]);
  ...
  it("replacing the follower's app drops its stale term", () => {
    ...
    expect(wb.execute(commands.replace(placement, "counter")).ok).toBe(true);
    expect(wb.core.getState().document.documents[LINKS_DOC_ID]).toBeUndefined();
  });
  ...
  it("cloning a workspace re-keys the terms onto the copies", () => {
    ...
    const copy = wb.execute(commands.cloneWorkspace(workspaceId));
    expect(copy.ok && copy.workspaceId).toBeTruthy();
    const bindings = bindingsOf(wb.core.getState().document);
  ...
  it("restore() brings the payload back and a foreign term is dropped on read", () => {
    ...
    expect(bindingsOf(again.core.getState().document).get(`${detail}/order`)).toMatchObject({ kind: "follow" });
    ...
    expect([...bindingsOf(again.core.getState().document).keys()]).toEqual([`${detail}/order`]);
  });
});
