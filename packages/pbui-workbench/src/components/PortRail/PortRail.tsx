 ...
 */
export function PortRail({ view, placementId, renderPort }: PortRailProps) {
  ...
  const one = (port: PortDefinition, side: "in" | "out") => {
    ...
    const content = renderPort && ref ? (
      <div key={`${side}:${port.id}`} className={styles.slot}>
        {renderPort(ref, node)}
      </div>
    ) : node;
    return <RegisteredCard key={`${side}:${port.id}`} placementId={placementId} portId={port.id} side={side}>{content}</RegisteredCard>;
  };
