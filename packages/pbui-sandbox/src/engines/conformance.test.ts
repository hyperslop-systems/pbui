...
/**
 * The engine conformance suite. Every engine must pass it unchanged; Phase 5
 * adds `["quickjs", () => createQuickJsEngine()]` to the table.
 */
const ENGINES: Array<[string, (limits?: ReturnType<typeof withLimits>) => ProgramEngine]> = [["eval", createEvalEngine]];
...
describe.each(ENGINES)("%s engine", (_name, make) => {
  ...
  test("fails to load a program that reaches for the DOM", async () => {
    const e = engine();
    await expect(e.load({ instanceId: "x1", programId: "dom", source: DOM_PROGRAM })).rejects.toThrow(/document is not available inside a program/);
  ...
  test("reports a syntax error at load, a missing definePlugin, and a duplicate instance", async () => {
    const e = engine();
    await expect(e.load({ instanceId: "s1", programId: "s", source: "definePlugin(({ ui }) => ({ title: 'x', widgets: { main: { render() { return ui.text(<b>x</b>); } } } }))" })).rejects.toThrow(SyntaxError);
