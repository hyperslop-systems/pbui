...
- [x] Step 2: write the intern guide — analysis (systems, the reactive sandbox pattern, gaps), design (dialect, engines, renderer, library, actions, tools, safety, decisions), phased implementation, sequences, failure modes, tests, API and file references <!-- t:0uc8 -->
- [x] Step 3: relate files, update changelog, run docmgr doctor, upload the guide bundle to reMarkable, final diary step <!-- t:hw81 -->
- [x] Phase 0 (impl): packages/pbui-sandbox skeleton — contracts, bootstrap, eval engine, validators, ported tests <!-- t:qayo -->
- [x] Phase 1 (impl): UINode→PBUI renderer, ProgramLibrary (localStorage), host loop hook, the script app; demo registration <!-- t:lycp -->
